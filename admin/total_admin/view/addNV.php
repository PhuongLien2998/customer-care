...
if (isset($_POST['submit']) ) {
   ...
   // check ten
    ...
    // check barnd
     if($barnd == ""){
            $eror_barnd = "chi nhánh không được để trống!";
            $error ++;
    }
	...
	<div class="row">
		<div class="col-md-6 col-md-offset-5"><br>
			...
			<form class="form-horizontal" enctype="multipart/form-data" role="form" method="POST" action=""  >
				...
				<div class="form-group">
					<label for="Brand" class="col-sm-3 control-label">Chi nhánh <span style="color: red;"><?php if(isset($eror_Barnd)){ echo $eror_Barnd; } ?></span></label>
					<select name="Barnd" class="browser-default custom-select custom-select-lg mb-3 select">
						<option selected  >Chọn chi nhánh </option>
						<?php
						while ($rowcn = mysqli_fetch_array($querycn)) {
							?>
							<option value="<?php  echo $rowcn['id_chinhanh']; ?>"> <?php  echo $rowcn[1]; ?>
						</option>
					<?php } ?>
				</select>
			    </div>
